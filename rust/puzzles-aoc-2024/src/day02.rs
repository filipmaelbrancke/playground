...
fn is_safe_report(report: &Vec<i32>) -> bool {
    ...
    let is_either_increasing_or_decreasing = differences.iter().all(|&x| x > 0) || differences.iter().all(|&x| x < 0);
    let differs_by_at_least_one_and_at_most_three = differences.iter().all(|&x| x.abs() >= 1 && x.abs() <= 3);
